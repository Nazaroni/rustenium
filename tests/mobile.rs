...
async fn test_create_ios_session() {
    ...
    // This will fail unless Rustenium-mobile and a simulator are available, but ensures compilation and call
...
async fn test_create_android_session() {
    ...
    // This will fail unless Rustenium-mobile and an emulator are available, but ensures compilation and call
    assert!(result.is_err() || result.is_ok());
}
