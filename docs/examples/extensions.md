...
async fn main() {
    ...
    let resp = bidi.send_command("example.method", serde_json::json!({"foo": "bar"}))
        .await
        .unwrap();
...
#[tokio::main]
async fn main() {
    let capabilities = BrowserCapabilities::Chrome(ChromeCapabilities::new())
        .to_value();
    let session = create_grid_session("http://localhost:4444/wd/hub", capabilities)
        .await
        .unwrap();
...
async fn main() {
    ...
    let capabilities = BrowserCapabilities::Chrome(ChromeCapabilities::new())
        .to_value();
    let session = WebDriverSession::create(client.clone(), capabilities)
        .await
        .unwrap();
